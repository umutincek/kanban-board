...
export const BoardContextProvider = ({
  ...
  const [displayAddEditBoard, setDisplayAddEditBoard] = useState({
    display: false,
    mode: "",
  });
  const [addBoardInputs, setAddBoardInputs] = useState<{
    name: string;
  ...
  const [editBoardInputs, setEditBoardInputs] = useState<{
    name: string;
    columns: { id: string; name: string }[];
  }>({
    name: "",
  ...
  const onChangeAddBoards = (
    ...
    if (mode === "deleting") {
      ...
      addBoardInputs.columns.forEach((column) => {
        newId =
          parseInt(newId) <= parseInt(column.id)
            ? `${parseInt(column.id) + 1}`
            : newId;
        newColumns.push({ ...column });
      });
  ...
  const onChangeEditBoards = (
    ...
    if (mode === "changeName") {
      ...
      const indexToRemove = editBoardInputs.columns.findIndex(
        (col) => col.id === columnId
      );
      const newColumns: {
        id: string;
      ...
      editBoardInputs.columns.forEach(
        (col: { id: string; name: string }, index) => {
