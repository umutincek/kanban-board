  ...
  return (
    <>
      ...
      <div className="w-[480px] min-h-[229px] flex flex-col justify-between p-8 rounded-md bg-white dark:bg-darkBg absolute top-2/4 left-2/4 -translate-x-2/4 -translate-y-2/4 z-20">
        <p className="text-hL text-red font-bold">
          Delete this {displayDeleteModal.mode} ?
        </p>
        {displayDeleteModal.mode === "board" && (
          <p className="text-bL text-mediumGrey">
            Are you sure you want to delete the ‘{completeBoardSelected?.name}’
            board? This action will remove all columns and tasks and cannot be
            reversed.
          </p>
        )}
        {displayDeleteModal.mode === "task" &&
          completeTaskSelected !== null && (
            <p className="text-bL text-mediumGrey">
              Are you sure you want to delete the ‘{completeTaskSelected?.title}
        ...
        <div className="w-full flex justify-between items-center">
          <button
            onClick={() => deleteThis()}
            className="w-[200px] h-[40px] text-bL font-bold bg-green/10 hover:bg-green/25 text-green rounded-[20px]"
          >
            Delete
          </button>
          <button
            ...
            className="w-[200px] h-[40px] text-bL font-bold bg-purple/10 dark:bg-white hover:bg-purple/25 text-purple rounded-[20px]"
          >
            Cancel
          </button>
        </div>
